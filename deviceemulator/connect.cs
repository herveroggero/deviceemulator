...
using deviceemulator.Properties;
using System;
...
namespace ui
{
    public partial class connect : Form
    {
        ...
        {
            string connStr = deviceemulator.Properties.Settings.Default.EnzoConnection;
            if (!string.IsNullOrEmpty(connStr))
            {
                try
                {
                    byte[] conn = System.Convert.FromBase64String(connStr);
                    EnzoHttpAzureIoTHub http = EnzoHttpAzureIoTHub.Deserialize(conn);
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                EnzoHttpAzureIoTHub http = new EnzoHttpAzureIoTHub(textBoxEnzoURI.Text, textBoxAuthToken.Text, textBoxConfigName.Text);
                ...
                {
                    Settings.Default.EnzoConnection = System.Convert.ToBase64String(conn);
                    Settings.Default.Save();
                }
                else
                {
                    Settings.Default.EnzoConnection = "";
                    Settings.Default.Save();
                }
